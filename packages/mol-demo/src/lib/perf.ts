...
export function isPerfEnabled(): boolean {
  ...
  try {
    return import.meta.env.DEV === true && new URLSearchParams(window.location.search).has("perf");
  } catch {
    return false;
  }
}
...
export function isRotateDisabled(): boolean {
  ...
  try {
    return (
      import.meta.env.DEV === true && new URLSearchParams(window.location.search).has("norotate")
    );
  } catch {
    return false;
  }
